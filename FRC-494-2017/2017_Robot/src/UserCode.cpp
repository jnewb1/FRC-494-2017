...
float a2_rotate = 250;
float a1_rotate = 0;
float a6_StrafeTime = 0;
float a5_StrafeTime = 0;
float a3_StrafeTime = 0;
float a4_StrafeTime = 0;
float a2_StrafeTime = 0;
float a1_StrafeTime = 0;
float atonNum = 1;
...
float shoesOut2 = 0.7;
float shoesIn2 = -0.7;
float rightShiftLow = 0.25;
float rightShiftHigh = 0.7;
float leftShiftLow = 0.25;
float leftShiftHigh = 0.7;

float curHotRatio = 1;
float minHotRatio = 0.05;


float RS_ShotAngBeg = 0;
float RS_ShotAngEnd = 0;

float armState = 0;
float armRate = 1;


float RS_ShotSpeed = 0;
float shooterState = 0;

float A_ShotAngBeg = 0;
float A_ShotAngEnd = 0;
float A_ShotSpeed = 0;
float B_ShotAngBeg = 0;
float B_ShotAngEnd = 0;
float B_ShotSpeed = 0;
float X_ShotAngBeg = 0;
float X_ShotAngEnd = 0;
float X_ShotSpeed = 0;
float Y_ShotAngBeg = 0;
float Y_ShotAngEnd = 0;
float Y_ShotSpeed = 0;
...
float sShotAngle2 = 0.3;
float sShotRate2 = 0.3;
float shooterMidDuration = 0.3;
float shooterTopDuration = 0.3;

float shooterDownDuration = 0.2;
float shooterDownRate = 0.2;
float shooterEndDuration = 0.2;
float shooterEndRate = 0.2;
...
//float crossHair1=314246;
//float crossHair2=285197;
//float crossHair3=272173;
//float crossHair4=270161;

// 70 Default Crosshairs

float crossHair1 = 324295;
float crossHair2 = 294224;
float crossHair3 = 270192;
float crossHair4 = 249185;
...
void userInit()
{
	robotTimeBase = (double)frc::GetClock(); //Time();
...
void userFastLoop(void)  //200HZ
{
	...
	double nTime = (double)frc::GetClock(); //GetTime();
...
if (match("a2_rotate"           ,a2_rotate)) return 1;
if (match("a1_rotate"           ,a1_rotate)) return 1;
if (match("a6_StrafeTime"       ,a6_StrafeTime)) return 1;
if (match("a5_StrafeTime"       ,a5_StrafeTime)) return 1;
if (match("a3_StrafeTime"       ,a3_StrafeTime)) return 1;
if (match("a4_StrafeTime"       ,a4_StrafeTime)) return 1;
if (match("a2_StrafeTime"       ,a2_StrafeTime)) return 1;
if (match("a1_StrafeTime"       ,a1_StrafeTime)) return 1;
...
if (match("atonNum", tmp)) { atonNum = (int)tmp; printf("setting atonnum\n"); return 1; }
   ...
   //   if (match("offsetBack"         ,offsetBack    )) return 1;
	...
	if (match("elevGain", elevGain)) return 1;
	if (match("feederGain", feederGain)) return 1;

	if (match("elevRes", elevRes)) return 1;
	if (match("elevOff", elevOff)) return 1;

	if (match("armRes", armRes)) return 1;
	if (match("armOff", armOff)) return 1;

	if (match("feederRes", feederRes)) return 1;
	...
	if (match("butterflyLocation", butterflyLocation)) return 1;
	...
	if (match("iArm", iArm)) return 1;
	if (match("iArmMax", iArmMax)) return 1;
	if (match("iArmMin", iArmMin)) return 1;
	if (match("iArmGain", iArmGain)) return 1;
	if (match("shoesOut2", shoesOut2)) return 1;
	if (match("shoesIn2", shoesIn2)) return 1;
	if (match("rightShiftLow", rightShiftLow)) return 1;
	if (match("rightShiftHigh", rightShiftHigh)) return 1;
	if (match("leftShiftLow", leftShiftLow)) return 1;
	if (match("leftShiftHigh", leftShiftHigh)) return 1;

	//if (match("A_ShotAngBeg" 	     ,A_ShotAngBeg 	       )) return 1;
	//if (match("A_ShotAngEnd"  	     ,A_ShotAngEnd  	       )) return 1;
	//if (match("A_ShotSpeed"   	     ,A_ShotSpeed   	       )) return 1;
	//if (match("B_ShotAngBeg" 	     ,B_ShotAngBeg 	       )) return 1;
	//if (match("B_ShotAngEnd"  	     ,B_ShotAngEnd  	       )) return 1;
	//if (match("B_ShotSpeed"   	     ,B_ShotSpeed   	       )) return 1;
	//if (match("X_ShotAngBeg" 	     ,X_ShotAngBeg 	       )) return 1;
	//if (match("X_ShotAngEnd"  	     ,X_ShotAngEnd  	       )) return 1;
	//if (match("X_ShotSpeed"   	     ,X_ShotSpeed   	       )) return 1;
	//if (match("Y_ShotAngBeg" 	     ,Y_ShotAngBeg 	       )) return 1;
	//if (match("Y_ShotAngEnd"  	     ,Y_ShotAngEnd  	       )) return 1;
	//if (match("Y_ShotSpeed"   	     ,Y_ShotSpeed   	       )) return 1;
	...
	if (match("RS_ShotSpeed", RS_ShotSpeed)) return 1;
	if (match("shooterState", shooterState)) return 1;
	...
	if (match("a1_Dist", a1_Dist)) return 1;
	if (match("a2_Dist", a2_Dist)) return 1;
	if (match("a3_Dist", a3_Dist)) return 1;
	if (match("a4_Dist", a4_Dist)) return 1;
	if (match("a5_Dist", a5_Dist)) return 1;
	if (match("a6_Dist", a6_Dist)) return 1;
	if (match("a1_Speed", a1_Speed)) return 1;
	if (match("a2_Speed", a2_Speed)) return 1;
	if (match("a3_Speed", a3_Speed)) return 1;
	if (match("a4_Speed", a4_Speed)) return 1;
	if (match("a5_Speed", a5_Speed)) return 1;
	if (match("a6_Speed", a6_Speed)) return 1;
	if (match("a1_BegA", a1_BegA)) return 1;
	if (match("a2_BegA", a2_BegA)) return 1;
	if (match("a3_BegA", a3_BegA)) return 1;
	if (match("a4_BegA", a4_BegA)) return 1;
	if (match("a5_BegA", a5_BegA)) return 1;
	if (match("a6_BegA", a6_BegA)) return 1;
	if (match("a1_EndA", a1_EndA)) return 1;
	if (match("a2_EndA", a2_EndA)) return 1;
	if (match("a3_EndA", a3_EndA)) return 1;
	if (match("a4_EndA", a4_EndA)) return 1;
	if (match("a5_EndA", a5_EndA)) return 1;
	if (match("a6_EndA", a6_EndA)) return 1;
	if (match("a1_Coll", a1_Coll)) return 1;
	if (match("a2_Coll", a2_Coll)) return 1;
	if (match("a3_Coll", a3_Coll)) return 1;
	if (match("a4_Coll", a4_Coll)) return 1;
	if (match("a5_Coll", a5_Coll)) return 1;
	if (match("a6_Coll", a6_Coll)) return 1;
	if (match("a1_Rate", a1_Rate)) return 1;
	if (match("a2_Rate", a2_Rate)) return 1;
	if (match("a3_Rate", a3_Rate)) return 1;
	if (match("a4_Rate", a4_Rate)) return 1;
	if (match("a5_Rate", a5_Rate)) return 1;
	if (match("a6_Rate", a6_Rate)) return 1;
	if (match("a1_cTime", a1_cTime)) return 1;
	if (match("a2_cTime", a2_cTime)) return 1;
	if (match("a3_cTime", a3_cTime)) return 1;
	if (match("a4_cTime", a4_cTime)) return 1;
	if (match("a5_cTime", a5_cTime)) return 1;
	if (match("a6_cTime", a6_cTime)) return 1;
	...
	if (match("ballDetect1", ballDetect1)) return 1;
	if (match("ballDetect2", ballDetect2)) return 1;
	if (match("ballDetect3", ballDetect3)) return 1;
	if (match("ballDetect4", ballDetect4)) return 1;
	if (match("armFlag", armFlag)) return 1;
	if (match("armSpeed", armSpeed)) return 1;
	if (match("elevSpeed", elevSpeed)) return 1;
   ...
   //   if (match("lightsOnTimeInZone"       ,lightsOnTimeInZone      )) return 1;
	...
	if (match("RS_ShotAngBeg", RS_ShotAngBeg)) return 1;
	if (match("RS_ShotAngEnd", RS_ShotAngEnd)) return 1;

	if (match("armState", armState)) return 1;
	if (match("armRate", armRate)) return 1;
	...
	if (match("crossHair1", crossHair1)) return 1;
	if (match("crossHair2", crossHair2)) return 1;
	if (match("crossHair3", crossHair3)) return 1;
	if (match("crossHair4", crossHair4)) return 1;




	if (match("ballLoadLoc", ballLoadLoc)) return 1;
	if (match("ballLoadRate", ballLoadRate)) return 1;
	if (match("fShotAngle1", fShotAngle1)) return 1;
	if (match("fShotRate1", fShotRate1)) return 1;

	if (match("fShotAngle2", fShotAngle2)) return 1;
	if (match("fShotRate2", fShotRate2)) return 1;
	if (match("sShotAngle1", sShotAngle1)) return 1;
	if (match("sShotRate1", sShotRate1)) return 1;

	if (match("sShotAngle2", sShotAngle2)) return 1;
	if (match("sShotRate2", sShotRate2)) return 1;
	if (match("shooterMidDuration", shooterMidDuration)) return 1;
	if (match("shooterTopDuration", shooterTopDuration)) return 1;

	if (match("shooterDownDuration", shooterDownDuration)) return 1;
	if (match("shooterDownRate", shooterDownRate)) return 1;
	if (match("shooterEndDuration", shooterEndDuration)) return 1;
	if (match("shooterEndRate", shooterEndRate)) return 1;
...
if (match("ShooterRPM1"         ,ShooterRPM1)) return 1;
if (match("ToolBarAmps"         ,ToolBarAmps)) return 1;
	if (match("AtonFinalRotate", AtonFinalRotate)) return 1;
	if (match("a6_ShotDist", a6_ShotDist)) return 1;
	if (match("a5_ShotDist", a5_ShotDist)) return 1;
	if (match("a4_ShotDist", a4_ShotDist)) return 1;
	if (match("a3_ShotDist", a3_ShotDist)) return 1;
	if (match("a2_ShotDist", a2_ShotDist)) return 1;
	if (match("a1_ShotDist", a1_ShotDist)) return 1;
	...
	if (match("gyro_Adjust", gyro_Adjust)) return 1;
...
void SaveGlobalData()
{
   ...
   //  fprintf(pFile, "eStepAngleUp          %f ", eStepAngleUp); eol(pFile);
	...
	fprintf(pFile, "elevRes   %f ", elevRes); eol(pFile);
	fprintf(pFile, "elevOff   %f ", elevOff); eol(pFile);
	fprintf(pFile, "elevGain  %f ", elevGain); eol(pFile);

	fprintf(pFile, "armRes    %f ", armRes); eol(pFile);
	fprintf(pFile, "armOff    %f ", armOff); eol(pFile);

	fprintf(pFile, "feederRes %f ", feederRes); eol(pFile);
  ...
  //fprintf (pFile, "gyroShift       %f ",gyroShift        ); eol(pFile);
	...
	//  fprintf (pFile, "butterflyLocation %f ",butterflyLocation); eol(pFile);
	...
	// fprintf (pFile, "iArm        %f ",iArm        ); eol(pFile);
	fprintf(pFile, "iArmMax         %f ", iArmMax); eol(pFile);
	fprintf(pFile, "iArmMin         %f ", iArmMin); eol(pFile);
	fprintf(pFile, "iArmGain        %f ", iArmGain); eol(pFile);
	fprintf(pFile, "shoesOut2         %f ", shoesOut2); eol(pFile);
	fprintf(pFile, "shoesIn2         %f ", shoesIn2); eol(pFile);
	fprintf(pFile, "rightShiftLow     %f ", rightShiftLow); eol(pFile);
	fprintf(pFile, "rightShiftHigh     %f ", rightShiftHigh); eol(pFile);
	fprintf(pFile, "leftShiftLow      %f ", leftShiftLow); eol(pFile);
	fprintf(pFile, "leftShiftHigh      %f ", leftShiftHigh); eol(pFile);


	//  fprintf (pFile, "A_ShotAngBeg      %f ",A_ShotAngBeg      ); eol(pFile);
	//  fprintf (pFile, "A_ShotAngEnd       %f ",A_ShotAngEnd       ); eol(pFile);
	//  fprintf (pFile, "A_ShotSpeed        %f ",A_ShotSpeed        ); eol(pFile);
	//  fprintf (pFile, "B_ShotAngBeg      %f ",B_ShotAngBeg      ); eol(pFile);
	//  fprintf (pFile, "B_ShotAngEnd       %f ",B_ShotAngEnd       ); eol(pFile);
	//  fprintf (pFile, "B_ShotSpeed        %f ",B_ShotSpeed        ); eol(pFile);
	////  fprintf (pFile, "X_ShotAngBeg      %f ",X_ShotAngBeg      ); eol(pFile);
	//  fprintf (pFile, "X_ShotAngEnd       %f ",X_ShotAngEnd       ); eol(pFile);
	//  fprintf (pFile, "X_ShotSpeed        %f ",X_ShotSpeed        ); eol(pFile);
	//  fprintf (pFile, "Y_ShotAngBeg      %f ",Y_ShotAngBeg      ); eol(pFile);
	//  fprintf (pFile, "Y_ShotAngEnd       %f ",Y_ShotAngEnd       ); eol(pFile);
	//  fprintf (pFile, "Y_ShotSpeed        %f ",Y_ShotSpeed        ); eol(pFile);

	//  fprintf (pFile, "RS_ShotAngBeg      %f ",RS_ShotAngBeg      ); eol(pFile);
	//  fprintf (pFile, "RS_ShotAngEnd      %f ",RS_ShotAngEnd       ); eol(pFile);
	//  fprintf (pFile, "RS_ShotSpeed       %f ",RS_ShotSpeed        ); eol(pFile);
	...
	fprintf(pFile, "gErrorLimit %f ", gErrorLimit); eol(pFile);
	...
	fprintf(pFile, "crossHair1      %f ", crossHair1); eol(pFile);
	fprintf(pFile, "crossHair2      %f ", crossHair2); eol(pFile);
	fprintf(pFile, "crossHair3      %f ", crossHair3); eol(pFile);
	fprintf(pFile, "crossHair4      %f ", crossHair4); eol(pFile);
	...
	fprintf(pFile, "a1_Dist       %f ", a1_Dist); eol(pFile);
	fprintf(pFile, "a2_Dist       %f ", a2_Dist); eol(pFile);
	fprintf(pFile, "a3_Dist       %f ", a3_Dist); eol(pFile);
	fprintf(pFile, "a4_Dist       %f ", a4_Dist); eol(pFile);
	fprintf(pFile, "a5_Dist       %f ", a5_Dist); eol(pFile);
	fprintf(pFile, "a6_Dist       %f ", a6_Dist); eol(pFile);
	fprintf(pFile, "a1_Speed      %f ", a1_Speed); eol(pFile);
	fprintf(pFile, "a2_Speed      %f ", a2_Speed); eol(pFile);
	fprintf(pFile, "a3_Speed      %f ", a3_Speed); eol(pFile);
	fprintf(pFile, "a4_Speed      %f ", a4_Speed); eol(pFile);
	fprintf(pFile, "a5_Speed      %f ", a5_Speed); eol(pFile);
	fprintf(pFile, "a6_Speed      %f ", a6_Speed); eol(pFile);
	fprintf(pFile, "a1_BegA        %f ", a1_BegA); eol(pFile);
	fprintf(pFile, "a2_BegA        %f ", a2_BegA); eol(pFile);
	fprintf(pFile, "a3_BegA        %f ", a3_BegA); eol(pFile);
	fprintf(pFile, "a4_BegA        %f ", a4_BegA); eol(pFile);
	fprintf(pFile, "a5_BegA        %f ", a5_BegA); eol(pFile);
	fprintf(pFile, "a6_BegA        %f ", a6_BegA); eol(pFile);
	fprintf(pFile, "a1_EndA       %f ", a1_EndA); eol(pFile);
	fprintf(pFile, "a2_EndA       %f ", a2_EndA); eol(pFile);
	fprintf(pFile, "a3_EndA       %f ", a3_EndA); eol(pFile);
	fprintf(pFile, "a4_EndA       %f ", a4_EndA); eol(pFile);
	fprintf(pFile, "a5_EndA       %f ", a5_EndA); eol(pFile);
	fprintf(pFile, "a6_EndA       %f ", a6_EndA); eol(pFile);
	fprintf(pFile, "a1_Coll      %f ", a1_Coll); eol(pFile);
	fprintf(pFile, "a2_Coll      %f ", a2_Coll); eol(pFile);
	fprintf(pFile, "a3_Coll      %f ", a3_Coll); eol(pFile);
	fprintf(pFile, "a4_Coll      %f ", a4_Coll); eol(pFile);
	fprintf(pFile, "a5_Coll      %f ", a5_Coll); eol(pFile);
	fprintf(pFile, "a6_Coll      %f ", a6_Coll); eol(pFile);
	fprintf(pFile, "a1_Rate     %f ", a1_Rate); eol(pFile);
	fprintf(pFile, "a2_Rate     %f ", a2_Rate); eol(pFile);
	fprintf(pFile, "a3_Rate     %f ", a3_Rate); eol(pFile);
	fprintf(pFile, "a4_Rate     %f ", a4_Rate); eol(pFile);
	fprintf(pFile, "a5_Rate     %f ", a5_Rate); eol(pFile);
	fprintf(pFile, "a6_Rate     %f ", a6_Rate); eol(pFile);
	fprintf(pFile, "a1_cTime      %f ", a1_cTime); eol(pFile);
	fprintf(pFile, "a2_cTime      %f ", a2_cTime); eol(pFile);
	fprintf(pFile, "a3_cTime      %f ", a3_cTime); eol(pFile);
	fprintf(pFile, "a4_cTime      %f ", a4_cTime); eol(pFile);
	fprintf(pFile, "a5_cTime      %f ", a5_cTime); eol(pFile);
	fprintf(pFile, "a6_cTime      %f ", a6_cTime); eol(pFile);
   ...
   //  fprintf(pFile,"liftFlag   %f ",liftFlag  ); eol(pFile);
	...
	//   fprintf(pFile,"ballDetect1       %f ",ballDetect1      ); eol(pFile);
	//   fprintf(pFile,"ballDetect2       %f ",ballDetect2      ); eol(pFile);
	//   fprintf(pFile,"ballDetect3       %f ",ballDetect3      ); eol(pFile);
	//   fprintf(pFile,"armFlag           %f ",armFlag          ); eol(pFile);
	fprintf(pFile, "armSpeed          %f ", armSpeed); eol(pFile);
	fprintf(pFile, "elevSpeed         %f ", elevSpeed); eol(pFile);
	...
	fprintf(pFile, "ballLoadLoc   %f ", ballLoadLoc); eol(pFile);
	fprintf(pFile, "ballLoadRate   %f ", ballLoadRate); eol(pFile);
	fprintf(pFile, "fShotAngle1   %f ", fShotAngle1); eol(pFile);
	fprintf(pFile, "fShotRate1   %f ", fShotRate1); eol(pFile);

	fprintf(pFile, "fShotAngle2  %f ", fShotAngle2); eol(pFile);
	fprintf(pFile, "fShotRate2     %f ", fShotRate2); eol(pFile);
	fprintf(pFile, "sShotAngle1     %f ", sShotAngle1); eol(pFile);
	fprintf(pFile, "sShotRate1      %f ", sShotRate1); eol(pFile);

	fprintf(pFile, "sShotAngle2 %f ", sShotAngle2); eol(pFile);
	fprintf(pFile, "sShotRate2      %f ", sShotRate2); eol(pFile);
	fprintf(pFile, "shooterMidDuration   %f ", shooterMidDuration); eol(pFile);
	fprintf(pFile, "shooterTopDuration   %f ", shooterTopDuration); eol(pFile);

	fprintf(pFile, "shooterDownDuration  %f ", shooterDownDuration); eol(pFile);
	fprintf(pFile, "shooterDownRate      %f ", shooterDownRate); eol(pFile);
	fprintf(pFile, "shooterEndDuration   %f ", shooterEndDuration); eol(pFile);
	fprintf(pFile, "shooterEndRate       %f ", shooterEndRate); eol(pFile);
...
fprintf (pFile, "a6_StrafeTime %f ",a6_StrafeTime); eol(pFile);
fprintf (pFile, "a5_StrafeTime %f ",a5_StrafeTime); eol(pFile);
fprintf (pFile, "a3_StrafeTime %f ",a3_StrafeTime); eol(pFile);
fprintf (pFile, "a4_StrafeTime %f ",a4_StrafeTime); eol(pFile);
fprintf (pFile, "a2_StrafeTime %f ",a2_StrafeTime); eol(pFile);
fprintf (pFile, "a1_StrafeTime %f ",a1_StrafeTime); eol(pFile);
...
fprintf (pFile, "ShooterRPM1 %f ",ShooterRPM1); eol(pFile);
fprintf (pFile, "ToolBarAmps %f ",ToolBarAmps); eol(pFile);
	fprintf(pFile, "AtonFinalRotate %f ", AtonFinalRotate); eol(pFile);
	fprintf(pFile, "a6_ShotDist %f ", a6_ShotDist); eol(pFile);
	fprintf(pFile, "a5_ShotDist %f ", a5_ShotDist); eol(pFile);
	fprintf(pFile, "a4_ShotDist %f ", a4_ShotDist); eol(pFile);
	fprintf(pFile, "a3_ShotDist %f ", a3_ShotDist); eol(pFile);
	fprintf(pFile, "a2_ShotDist %f ", a2_ShotDist); eol(pFile);
	fprintf(pFile, "a1_ShotDist %f ", a1_ShotDist); eol(pFile);
...
void GlobalSendBack(void)
{
  ...
  */
  ...
  sendBack(191,a6_rotate);
  sendBack(181,a5_rotate);
  sendBack(192,a4_rotate);
  sendBack(197,a3_rotate);
  sendBack(245,a2_rotate);
  sendBack(243,a1_rotate);
  ...
  sendBack(180,a6_StrafeTime);
  sendBack(183,a5_StrafeTime);
  sendBack(186,a3_StrafeTime);
  sendBack(185,a4_StrafeTime);
  sendBack(187,a2_StrafeTime);
  sendBack(189,a1_StrafeTime);
...
void ZeroHood() { tprintf("ZeroGyro\n"); resetGyro(); }
...
void updateDashboardLights()
{
	...
	{
		if (inDisable) matchTimeBase = frc::GetClock() - matchTime;
		else
			matchTime = frc::GetClock() - matchTimeBase;
	}

	if (matchStatus == 1 && inTele && !inDisable && matchTime > 10) { matchStatus = 2; matchTimeBase = frc::GetClock() - 10; }
...
void UDCReceiverTest()
{
	...
	if (result != -1)
	{
		...
		lastUDC = frc::GetClock();

	}
	else
	{

		if (frc::GetClock() - lastUDC > 2) //socket timeout (reset socket)
		{
			close(hSocket);
...
void UDCReceiverTestfrompi()
{
	...
	if (result != -1)
	{
		...
		lastUDCfrompi = frc::GetClock();

	}
	else
	{

		if (frc::GetClock() - lastUDCfrompi > 2) //socket timeout (reset socket)
		{
			close(hSocketfrompi);
