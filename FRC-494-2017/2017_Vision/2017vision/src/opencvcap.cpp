...
void setupcam1(){
	cap1.set(CV_CAP_PROP_FRAME_WIDTH,320); //1280); //320);
    cap1.set(CV_CAP_PROP_FRAME_HEIGHT,240); //720); //240);
    cap1.set(CV_CAP_PROP_BRIGHTNESS,0.1);
    cap1.set(CV_CAP_PROP_FPS,30);
    
    
}
void setupcam2(){
	cap2.set(CV_CAP_PROP_FRAME_WIDTH,320); //1280); //320);
    cap2.set(CV_CAP_PROP_FRAME_HEIGHT,240); //720); //240);
    cap2.set(CV_CAP_PROP_BRIGHTNESS,0.01);
...
printf("Cameras Opened Sucessfully\n");
     //Mat edges;
     Mat cam1frame;
     Mat cam2frame;
     Mat frame;    
	...
	//printf("%d %d %f\n",quality,frameSize,  1.0/(curClock-lastClock));

        lastClock=curClock;
		
       // double brite=cap.get(CV_CAP_PROP_BRIGHTNESS);
        //double fps=cap.get(CV_CAP_PROP_FPS);
    
        //printf("brite %f  Fps %f   ",brite,fps);
        
	
		if (!cap1.isOpened()) { cap1.open("/dev/v4l/by-id/usb-KYE_Systems_Corp._USB_Camera_200901010001-video-index0"); setupcam1(); }
		if (!cap2.isOpened()) { cap2.open("/dev/v4l/by-id/usb-Microsoft_Microsoft®_LifeCam_HD-3000-video-index0"); setupcam2(); }
	...
	else if (selectedCam == 2 && !cap2.isOpened()) { selectedCam=1; }

	if(selectedCam == 1 && cap1.isOpened()){ cap1 >> cam1frame; }
    else if(selectedCam == 2 && cap2.isOpened()){ cap2 >> cam2frame; } 
	else { continue; }
	//Size size(320,240);
	...
	*/
		...
		cv::imencode(".jpeg", frame, buff, param);
                ...
                if (frameSize<7500) quality+=3; else quality-=10;
		...
		rioBuf[2] = foundGoal;
		rioBuf[3] = bestPegCenterX;
		rioBuf[4] = bestPegCenterY;
		rioBuf[5] = foundPeg;
		rioBuf[6] = curFPS;
		rioBuf[7] = bestPegAngle;
